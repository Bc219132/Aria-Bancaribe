...
{
    public class ReclamoPOSFromAllClaims : CancelDialog
    {
        ...
        public ReclamoPOSFromAllClaims(ISendGridEmailService sendGridEmailService, PrivateConversationState userState)
            : base(nameof(ReclamoPOSFromAllClaims))
        {
        ...
        private async Task<DialogTurnResult> SetSecondAlt(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                await stepContext.Context.SendActivityAsync($"{ClientData.name}, ¿posees algún otro número de contacto adicional al {ClientData.phone}, donde podamos contactarte?");
            ...
            if (string.IsNullOrEmpty(ClientData.phoneAlt))
            {
                if (stepContext.Context.Activity.Text.ToString().ToLower().Equals("si"))
                {
                    ...
                    return await stepContext.PromptAsync(
                        ...
                        new PromptOptions
                        {
                            Prompt = MessageFactory.Text("Ingresa el número de teléfono incluyendo el código de área u operadora ejemplo: 04240000000 o 02120000000"),
                            RetryPrompt = MessageFactory.Text("Por favor ingresa el número de teléfono sin caracteres especiales:")
                        },
                        cancellationToken
                        );
                }
