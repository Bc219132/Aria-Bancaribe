...
namespace BanCoreBot.Dialogs.Reclamos
{
    public class ReclamoTransferencia : CancelDialog
    {
        ...
        public ReclamoTransferencia(ILuisService luisService, ISendGridEmailService sendGridEmailService, PrivateConversationState userState)
            : base(nameof(ReclamoTransferencia))
        {
            ...
            AddDialog(new TextPrompt(_InitDatosInic));
        ...
        private async Task<DialogTurnResult> InitDatosInic(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                await stepContext.Context.SendActivityAsync($"Entiendo, cuando requieras crear alguno puedes contar conmigo.", cancellationToken: cancellationToken);
        ...
        private async Task<DialogTurnResult> SetSecondAlt(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Context.Activity.Text.ToLower().Equals("No enviada a otro banco"))
            {
                ClientData.claimName = "Formulario Transferencia no enviada a otro banco";
                ClientData.code = "6.02";
            }
            else
            {
                ClientData.claimName = "Formulario transferencia duplicada";
                ClientData.code = "6.05";
            }
        ...
        private async Task<DialogTurnResult> Set4DigCta(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(
                ...
                new PromptOptions
                {
                    Prompt = MessageFactory.Text("Por favor ingrese los últimos cuatro (4) dígitos de la cuenta del cliente:"),
                    RetryPrompt = MessageFactory.Text("El dato ingresado no es válido, por favor verifique e ingrese solo los cuatro (4) últimos números de la cuenta del cliente:")
                },
                cancellationToken
                );
        }
        ...
        {
            if (promptContext.Context.Activity.Text.ToLower().Equals("no enviada a otro banco") ||
                promptContext.Context.Activity.Text.ToLower().Equals("duplicada"))
            {
                return Task.FromResult(true);
            }
            else
        ...
        {
            var validate = utilitario.ValidateDate5a120(promptContext.Context.Activity.Text);
            if (validate == 0)
            {
                return Task.FromResult(true);
            }
            ...
            {
                promptContext.Context.SendActivityAsync($"Deben de pasar al menos 5 días a partir de la fecha de operación para realizar el reclamo {Environment.NewLine}" +
        ...
        private Activity CreateButtonsClaims(string retry)
        {
            ...
            {
                reply = MessageFactory.Text("¿Desea crear un reclamo relacionado con transferencias?");
            }
            else
            {
                reply = MessageFactory.Text("Por favor indique una respuesta, ¿Desea crear un reclamo relacionado con transferencias?");
            }
        ...
        private Activity CreateButtonsTypeClaim(string retry)
        {
            ...
            reply.SuggestedActions = new SuggestedActions()
            {
                Actions = new List<CardAction>()
                {
                    new CardAction() {Title = "No enviada a otro banco", Value = "No enviada a otro banco" , Type = ActionTypes.ImBack},
                    new CardAction() {Title = "Duplicada", Value = "Duplicada" , Type = ActionTypes.ImBack}
