...
{
    public class ReclamoCajero : CancelDialog
    {
        ...
        public ReclamoCajero(ISendGridEmailService sendGridEmailService, PrivateConversationState userState)
            : base(nameof(ReclamoCajero))
        {
        ...
        private async Task<DialogTurnResult> SetSecondAlt(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (stepContext.Context.Activity.Text.ToLower().Equals("retiro en cajero automático bancaribe"))
            {
                ClientData.claimName = "Formulario Cajeros Automáticos Bancaribe";
                ClientData.code = "1.01";
            }
            else
            {
                ClientData.claimName = "Formulario Cajeros Automáticos Otro Banco";
                ClientData.code = "1.03";
            }
        ...
        private async Task<DialogTurnResult> SetMonto(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            return await stepContext.PromptAsync(
                ...
                new PromptOptions
                {
                    ...
                    RetryPrompt = MessageFactory.Text("Por favor ingrese el monto del reclamo sin letras ni caracteres especiales:")
                },
                cancellationToken
                );
        }
        ...
        private async Task<DialogTurnResult> SetNombreOtroBanco(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            if (ClientData.code.Equals("1.03"))
            {
                return await stepContext.PromptAsync(
                    ...
                    new PromptOptions
                    {
                        Prompt = MessageFactory.Text("Por favor ingrese el nombre del banco al que pertence el cajero donde realizó la operación:"),
                        RetryPrompt = MessageFactory.Text("Por favor ingrese el nombre del banco al que pertence el cajero donde realizó la operación:")
                    },
                    cancellationToken
                    );
            }
        ...
        private async Task sendEmail(WaterfallStepContext stepContext)
        {
            ...
            ClientData = await userStateAccessors.GetAsync(stepContext.Context, () => new UserPersonalData());
            string contentEmail = "";
            if (!string.IsNullOrEmpty(ClientData.nameOtherBank) && ClientData.hasOtherPhone)
            {
                contentEmail = $"Un usuario ha solicitado la creación de un reclamo, a continuación la información recopilada en la conversación {Environment.NewLine}" +
                  ...
                  $"{Environment.NewLine} 🏦 Nombre del banco: {ClientData.nameOtherBank}" +
                  $"{Environment.NewLine} 📄 Descripción: {ClientData.Description}";
            }
            else if (!string.IsNullOrEmpty(ClientData.nameOtherBank) && !ClientData.hasOtherPhone)
            {
                contentEmail = $"Un usuario ha solicitado la creación de un reclamo, a continuación la información recopilada en la conversación {Environment.NewLine}" +
                  $"{Environment.NewLine} {Environment.NewLine} 🖊 Nombre del reclamo: {ClientData.claimName} " +
                  $"{Environment.NewLine} 📌 Código del reclamo: {ClientData.code} " +
        ...
        {
            if (promptContext.Context.Activity.Text.ToLower().Equals("retiro en cajero automático bancaribe") ||
                promptContext.Context.Activity.Text.ToLower().Equals("retiro en cajero automático de otro banco de suiche"))
            {
                return Task.FromResult(true);
            }
            else
        ...
        private Activity CreateButtonsClaims(string retry)
        {
            ...
            {
                reply = MessageFactory.Text("¿Desea crear un reclamo relacionado con operaciones en cajeros automáticos?");
            }
            else
            {
                reply = MessageFactory.Text("Por favor indique una respuesta, ¿Desea crear un reclamo relacionado con operaciones en cajeros automáticos?");
            }
        ...
        private Activity CreateButtonsTypeClaim(string retry)
        {
            ...
            reply.SuggestedActions = new SuggestedActions()
            {
                Actions = new List<CardAction>()
                {
                    new CardAction() {Title = "Retiro en Cajero Automático Bancaribe", Value = "Retiro en Cajero Automático Bancaribe" , Type = ActionTypes.ImBack},
                    new CardAction() {Title = "Retiro en Cajero Automático de otro banco de Suiche", Value = "Retiro en Cajero Automático de otro banco de Suiche" , Type = ActionTypes.ImBack}
